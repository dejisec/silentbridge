...
class cert_cnf(object):
    ...
    @classmethod
    def configure(
        cls, country=None, state=None, locale=None, org=None, email=None, cn=None
    ):

        with open(getattr(cls, "path"), "w", encoding="utf-8") as fd:
            fd.write(
                getattr(cls, "template") % (country, state, locale, org, email, cn)
            )
