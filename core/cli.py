...
def options():
    ...
    spoofing_params.add_argument(
        "--switch-mac", dest="switch_mac", type=str, help="MAC address of switch."
    )

    spoofing_params.add_argument(
        "--gw-mac", dest="gw_mac", type=str, help="MAC address of gateway."
    )

    spoofing_params.add_argument(
        "--client-ip", dest="client_ip", type=str, help="IP address of client device."
    )

    spoofing_params.add_argument(
        "--switch-ip", dest="switch_ip", type=str, help="IP address of upstream switch."
    )

    spoofing_params.add_argument(
        "--gw-ip", dest="gw_ip", type=str, help="IP address of the upstream gateway."
    )
    ...
    interfaces_group.add_argument(
        "--upstream", dest="upstream", type=str, help="Upstream network interface."
    )

    interfaces_group.add_argument(
        "--phy", dest="phy", type=str, help="Downstream network interface."
    )
    ...
    bait_n_switch_group.add_argument(
        "--wired-conf",
        dest="wired_conf",
        type=str,
    ...
    splitterctl_group.add_argument(
        "--upstream-splitter",
    ...
    parser.add_argument(
        "--debug", dest="debug", action="store_true", help="start in debug mode"
    )
    ...
    if args.create_bridge and missing_nec_params:
        parser.error(
            "--create-bridge requires the --phy, --sidechannel, and --upstream flags"
        )
    ...
    if args.add_interaction and missing_nec_params:
        parser.error(
            "--add-interaction requires the --phy, --sidechannel, --upstream, --client-mac, --client-ip, and --gw-mac flags"
        )
    ...
    if args.rogue_gateway and missing_nec_params:
        parser.error(
            "--rogue-gateway requires --client-mac, --switch-mac, --phy, --netmask, --switch-ip"
        )
    ...
    if args.bait_n_switch and missing_nec_params:
        parser.error(
            "--bait-n-switch requires --client-mac, --upstream, --client-ip, --netmask --wired-conf"
        )
    ...
    if args.splitterctl and missing_nec_params:
        parser.error(
            "--splitterctl requires the --upstream-splitter and --phy-splitter flags"
        )
